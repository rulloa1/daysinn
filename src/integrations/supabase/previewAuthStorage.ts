...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      let done = false;
      // Assigned below rather than here: `finish` closes over `timer` to clear it,
      // and the timeout must not start until the request has been posted. `const`
      // cannot express that without moving the setTimeout call earlier, which would
      // change when the timeout begins.
